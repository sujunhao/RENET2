...
def evaluate_rst_all_info(ori_df, ori_c, ann_df, is_freeze_c = False, b_ann_df = False):
    global BeFree_df, DTMiner_df 
    _rst = []
#     _df = ReNet_df
#     _c = 'is_renet'   #is_renet | pred_10 | etc
#     ann_df = abs_s_df
...
#     print(merge_t_df.head(3))

    # precision on olp 3 & based #
#     ann_positive = ann_df[ann_df['new_label']==1]
    ann_positive = ann_df.copy()
    
    df=merge_t_df
    F=[1,1,1]
    c=[_c,'is_dtminer','is_befree']
#     print(c)
    if is_freeze_c:
        c=['is_renet','is_dtminer','is_befree']
    
    # c[0], c[2] = c[2], c[0]
    mdl_positive = df[(df[c[0]]==F[0]) & ((df[c[1]]==F[1]) & (df[c[2]]==F[2]))]
...
#     print(S2)
    _rst.extend([_precision2, N_p2])

    # precision on olp 2 & based #
#     ann_positive = ann_df[ann_df['new_label']==1]
    ann_positive = ann_df.copy()
...
#     print('model confidence GDA {}, adding {} to TP, total TP {}'.format(mdl_n, olp_n-ann_n, olp_n))
    ...
    if isinstance(b_ann_df, pd.DataFrame):
        ...
        tar_df['pred'] = _df.apply(lambda x: 1 if x[_c] ==1 else 0, axis=1)
    S3 = get_scores_a(tar_df, 'pred', targe_df, 'new_label')
    _precision3, _, N_p3 = S3
#     print(S3)
    _rst.extend([_precision3, N_p3])
...
#     _rst = _rst1[0], _rst1[2], _rst2[2], _rst1[4], _rst2[4], _rst1[6], _rst2[6]
    _rst = _rst1[0], _rst1[2], _rst2[2], _rst1[6], _rst2[6]
    return _rst
...
def renet2_train(args):
    ...
    args.label_f_name = 'labels_n2.txt'
    ...
    print('get the result of the RENET trained based models, for [enhanced data]')
    m_df = 0
    s_arr = []
    for _i in range(10):
    ...
    F=[0, 0, 0] #all negative
    # F=[1, 1, 1] #all positive
    c=['is_renet','is_dtminer','is_befree']
    df = merge_t_df
    mdl_df = df[(df[c[0]]==F[0]) & ((df[c[1]]==F[1]) & (df[c[2]]==F[2]))].copy()
    mdl_df['is_tar'] = 1
    ...
    #     cls_rst_file = os.path.join(args.raw_data_dir, "ft_rst_ann_2nd_dev_exp_w%d_%02d.tsv" % (IG_N, _i))
        ...
        cls_rst_file = os.path.join(args.raw_data_dir, "cls_rst/ft_rst_ann_2nd_dev_%02d.tsv" % (_i))
    ...
    s_arr = []
    for _i in range(10):
        cls_file = os.path.join(args.raw_data_dir, "cls_rst/ft_rst_ann_2nd_dev_%02d.tsv" % (_i+1))
